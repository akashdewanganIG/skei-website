...
export function LeadDetail({
  ...
  const campaignOptions = useMemo(() => campaignSourceOptions(categories), [categories]);
  const currentCampaign = inferCampaignSource(lead, categories).name;
  const selectedCampaign =
    campaignOptions.find((option) => option.value === currentCampaign) ??
    ({ value: currentCampaign, label: currentCampaign } as SelectOption);
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              ...
              <div className="mt-2">
                {canEditDetails ? (
                  <Select
                    instanceId="lead-campaign"
                    options={campaignOptions}
                    value={selectedCampaign}
                    isSearchable
                    onChange={(option: unknown) => {
                      const next = option as SelectOption | null;
                      if (next && next.value !== currentCampaign) {
                        onPatch(lead.id, { source: next.value });
                      }
                    }}
                  />
                ) : (
                  <span
                    ...
                    style={{
                      color: ORANGE_ACCENT,
                      backgroundColor: hexA(ORANGE_ACCENT, 0.1),
